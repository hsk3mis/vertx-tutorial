...
public class VerticleWithExternalConfiguration extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) throws Exception //startFuture let us inform Vert.x when out start sequence is completed or report an error.
    {
        vertx
            ...
            .listen(config().getInteger("http.port", 8080), result -> { //handler executed when server is actually started listening
                if (result.succeeded())
